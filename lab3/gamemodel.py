...
class Player:
   ...
   #HINT: It should probably take the Game that creates it as parameter and some additional properties that differ between players (like firing-direction, position and color)
    ...
    def fire(self, angle, velocity):
        ...
        if (self.isReversed): angle = 180 - angle
        wind = self.game.getCurrentWind()
        yPos = self.game.getCannonSize() / 2


        return Projectile(angle, velocity, wind, self.xPos, yPos, -110, 110)
    ...
    def projectileDistance(self, proj):
        ...
        cannonLeftEdge = self.xPos - cannonSize/2
        cannonRightEdge = self.xPos + cannonSize/2

        projLeftEdge = proj.getX() - ballSize
        projRightEdge = proj.getX() + ballSize

        if projRightEdge >= cannonLeftEdge and projLeftEdge <= cannonRightEdge:
            return 0
        elif projRightEdge < cannonLeftEdge:
            return projRightEdge - cannonLeftEdge
        elif projLeftEdge > cannonRightEdge:
            return projLeftEdge - cannonRightEdge
    ...
    def getAim(self):
        return self.prevAngle, self.prevVelocity
